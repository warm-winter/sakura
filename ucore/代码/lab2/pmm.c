...
// get_pte-获取pte并为la返回此pte的内核虚拟地址
//		  -如果此pte的PT内容不存在，则为PT分配一个页面
//参数：
// pgdir：PDT的内核虚拟基地址
// la：需要映射的线性地址
//创建：一个逻辑值，用于确定是否为PT分配页面
...
get_pte(pde_t *pgdir, uintptr_t la, bool create) {
     ...
     */
#if 0
    pde_t *pdep = NULL;   // (1) 查找页目录项 find page directory entry
    if (0) {              // (2) 检查输入是否不存在 check if entry is not present
                          // (3) 检查是否需要创建，然后为页表分配页 check if creating is needed, then alloc page for page table
                          // 注意：此页面用于页面表，而不用于公共数据页面 CAUTION: this page is used for page table, not for common data page
                          // (4) 设置页面参考 set page reference
        uintptr_t pa = 0; // (5) 获取页面的线性地址 get linear address of page
                          // (6) 使用memset清除页面内容 clear page content using memset
                          // (7) 设置页目录条目的权限 set page directory entry's permission
    }
    return NULL;          // (8) 返回页表项 return page table entry
...
// page_remove_pte-释放与线性地址la相关的Page sturct
//-和clean（invalidate）与线性地址la相关的pte
//注意：PT已更改，因此TLB需要无效
static inline void
page_remove_pte(pde_t *pgdir, uintptr_t la, pte_t *ptep) {
     ...
     */
#if 0
    if (0) {                      //(1)检查此页表项是否存在 check if this page table entry is present
        struct Page *page = NULL; //(2)找到对应的页面到pte find corresponding page to pte
                                  //(3)减少页面引用 decrease page reference
                                  //(4)并在页面引用达到0时释放此页面 and free this page when page reference reachs 0
                                  //(5)清除第二页表条目 clear second page table entry
                                  //(6)刷新tlb flush tlb
