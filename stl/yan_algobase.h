...
#ifndef SAKURA_YAN_ALGOBASE_H
#define SAKURA_YAN_ALGOBASE_H
//把常用的一些算法定义在<stl_algobase.h>只中，其他算法定义在<stl_algo.h>中。
...
__STL_BEGIN_NAMESPACE
    ...
    inline void _iter_swap(_ForwardIter1 __a,_ForwardIter2 __b,_Tp*){
    _Tp __tmp = *__a;
    *__a = *__b;
    *__b = __tmp;
}
...
//根据自己来设定的规则来比大小
//comp为1，表示第一个参数小
    template<class _Tp,class _Comparable>
...
//copy
    ...
    template<class _BidirectionalIter1,class _BidirectionalIter2,class _Distance>
    inline _BidirectionalIter2 __copy_backward(_BidirectionalIter1 __first,_BidirectionalIter2 __last,_BidirectionalIter2 __result,random_access_iterator_tag,_Distance *){
...
struct __copy_backward_dispatch<_Tp *,_Tp,__true_type>{
    ...
    template<class _InputIter,class _OutputIter>
    inline _OutputIter __copy_aux2(_InputIter __first,_InputIter __last,_OutputIter __result,){
        return __copy_trivial(__first,__last,__result);
    }

    template<class _Tp>
    inline _Tp * __copy_trivial(const _Tp *__first,const _Tp __last,_Tp * __result){
        mommove(__result,__first,sizeof(_Tp)*(__last - __first));
...
//在比较两个寄存器的大小,根据第一个元素比较
    ...
    inline bool lexicograhpcal_compare(const unsigned char *__first1,const unsigned char *__last1,const unsigned char *__first2,const unsigned char *__last2){
        const size_t __len1 = __last1 - __first1;
        const size_t __len2 = __last2 - __first2;
        const int __result = memcmp(__first1,__first2,min(__len1,__len2));
        return __result!=0?__result<0?:__len1<len2;
    }
    __STL_END_NAMESPACE
#endif //SAKURA_YAN_ALGOBASE_H
